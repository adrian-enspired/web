jQuery(function($){
	...
	$('.projects_carousel').waitForImages(function() {
		$('.projects_carousel').carouFredSel({
			width: "100%",
			circular: false,
			infinite: false,
			auto: false,
			...
			prev: { button: "#projects-prev", key: "left"},
			next: { button: "#projects-next",key: "right"},
		});
	});
	$('.artists_carousel').waitForImages(function() {
		$('.artists_carousel').carouFredSel({
			width: "100%",
			circular: false,
			infinite: false,
			auto: false,
			...
			prev: { button: "#artists-prev", key: "left"},
			next: { button: "#artists-next",key: "right"},
		});
	});
	...
	function IsEmail(email) {
		...
		$('.js-news-popup-open').click(function() {
			var button = $(this);
			var popup = $(button.data('popup-id'));
			$.ajax({
				url: '?ajax&type=news&id=' + button.data('id'),
				type: 'GET',
				data: {},
				success: function(data) {
					if (data && data.success) {
						popup.find('.js-news-popup-title').html(data.title);
						popup.find('.js-news-popup-author').html(data.name);
						popup.find('.js-news-popup-date').html(data.date);
						var image = popup.find('.js-news-popup-image');
						image.closest('ul').hide();
						if (data.image) {
							image.closest('ul').show();
							image.attr('src', data.image);
						}
						popup.find('.js-news-popup-body').html(data.body);
						setTimeout(function() {
							$('#news_pop').modal('show');
						}, 250);
					}
		...
		$('.js-preloader-button').click(function() {
			var button = $(this);
			var items = button.parent().parent().find('.hidden-item');
			items.css('display', 'block');
			items.animate({opacity: 1}, 3000);
			button.hide();
			return false;
		});

		function ajaxFormResponse(form, data) {
			form.find('input, textarea').attr('style', '');
			if (data && data.success) {
		...
		$('.js-subscribe').click(function() {
			...
			$.ajax({
				url: '?ajax&' + form.serialize(),
				type: 'GET',
				data: {},
				success: function(data) {
					ajaxFormResponse(form, data);
				},
				error: function (request, status, error) {
					console.log(request.responseText);
				}
		...
		$('.js-contacts').click(function() {
			...
			var form = button.closest('form');
			$.ajax({
				url: '?ajax&contactus=1&' + form.serialize(),
				type: 'GET',
				data: {},
				success: function(data) {
					ajaxFormResponse(form, data);
